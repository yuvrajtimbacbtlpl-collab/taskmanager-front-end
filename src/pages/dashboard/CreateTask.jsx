...
const HOUR_OPTIONS = [
  0.5, 1, 1.5, 2, 2.5, 3, 4, 5, 6, 7, 8, 10, 12, 16, 20, 24, 32, 40, 48, 60, 80,
  100,
];
...
function SchedulePreview({ estimatedHours, workingHours, holidays }) {
  ...
  useEffect(() => {
    if (!estimatedHours || !workingHours?.length) {
      setResult(null);
      return;
    }
    const r = calculateEndDate(
    ...
    >
      ...
      >
        ...
        >
          {totalHours}h
          {skippedDays > 0
            ? ` · ${skippedDays} off-day${skippedDays > 1 ? "s" : ""} skipped`
            : ""}
          {" · "}
            ...
            {breakdown.map((d, i) => (
              ...
              >
                ...
                >
                  <div
                    style={{
                      width: `${(d.hoursUsed / totalHours) * 100}%`,
                      background: d.partial ? "#fb923c" : "#22c55e",
                      height: "100%",
                      transition: "width .3s",
                    }}
                  />
...
function TaskDateBadge({ task }) {
  ...
  const isDone = ["completed", "done", "closed", "resolved"].some((s) =>
  ...
  return (
    ...
    >
      ...
      {endDate ? (
        ...
        >
          {isOverdue ? "⚠" : isSoon ? "⏰" : "🔴"} {fmtShort(endDate)}{" "}
          {fmtTime(endDate)}
        </span>
      ) : (
        <span
...
export default function CreateTask() {
  ...
  const [limit, setLimit] = useState(() =>
    Number(localStorage.getItem("table_entries") || 10),
  );
  ...
  const [viewMode, setViewMode] = useState(
    () => localStorage.getItem("task_view_mode") || "table",
  );
  ...
  useEffect(() => {
    if (companyId && companyId !== "global") {
      api(`/company/${companyId}`)
        .then((d) => {
          if (d?.workingHours) setCompanyData(d);
        })
        .catch(() => {});
    }
  }, [companyId]);
  ...
  const scheduleSummary = workingHours.length
    ? getScheduleSummary(workingHours)
    : null;
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (res?.data) {
        ...
        setTotalPages(Math.ceil(raw.length / cl) || 1);
        setTasks(raw.slice((page - 1) * cl, page * cl));
      }
    } catch {
      setTasks([]);
    } finally {
  ...
  useEffect(() => {
    if (canRead && (selectedProject?._id || isGlobal)) fetchTasks();
  ...
  useEffect(() => {
    if (canRead) {
      const sUrl =
        isAdmin && companyId
          ? `/task-status/active?company=${companyId}`
          : "/task-status/active";
      api(sUrl)
        .then((d) => setStatuses(d || []))
        .catch(() => {});
      const stUrl = companyId
        ? `/auth/staff?company=${companyId}`
        : "/auth/staff";
      api(stUrl)
        .then((d) => setStaff(d || []))
        .catch(() => {});
  ...
  useEffect(() => {
    if (selectedProject?._id) {
      ...
      const r = () => fetchTasks();
      socketService.onTaskCreated(r);
      socketService.onTaskUpdated(r);
      socketService.onTaskDeleted(r);
      socketService.onTaskStatusChanged(r);
      return () => {
        socketService.offTaskCreated?.(r);
        socketService.offTaskUpdated?.(r);
        socketService.offTaskDeleted?.(r);
        socketService.offTaskStatusChanged?.(r);
  ...
  const updateInline = async (row, updates) => {
    ...
    if (!canUpdate && !statusOnly) {
      showToast("Permission denied", "error");
      return;
    }
    try {
      setTasks((p) =>
        p.map((t) => (t._id === row._id ? { ...t, ...updates } : t)),
      );
      ...
      fd.append("appLink", window.location.href);
      await api(`/tasks/${row._id}`, { method: "PUT", body: fd });
      showToast(
        updates.status && !updates.assignedTo
          ? "Status updated"
          : updates.assignedTo && !updates.status
            ? "Assignee updated"
            : "Task updated",
        "update",
      );
  ...
  const handleFileChange = (e) =>
    setForm((f) => ({
      ...f,
      media: [...f.media, ...Array.from(e.target.files)],
    }));
  const removeMedia = (i) =>
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.assignedTo) {
      showToast("Please assign to a staff member", "error");
      return;
    }
    if (!form.status) {
  ...
  return (
    <div className="permission-page">
      ...
      {scheduleSummary && (
        ...
        >
          <Clock size={13} /> <strong>Company Schedule:</strong>{" "}
          {scheduleSummary}
        </div>
      )}
      ...
      {loading ? (
        ...
        <KanbanBoard
          ...
          onStatusChange={(row, newStatus) =>
            updateInline(row, { status: newStatus })
          }
        />
      ) : (
        <CommonTable
          columns={[
            ...
            {
              header: "Schedule",
              render: (row) => <TaskDateBadge task={row} />,
            },
            {
              header: "Status",
              render: (row) => (
                <select
                  ...
                  onChange={(e) =>
                    updateInline(row, { status: e.target.value })
                  }
                >
                  {statuses.map((s) => (
                    <option key={s._id} value={s.name}>
            ...
            },
            {
              ...
              render: (row) => (
                <select
                  ...
                  onClick={(e) => e.stopPropagation()}
                  onChange={(e) =>
                    updateInline(row, { assignedTo: e.target.value })
                  }
                >
      ...
      <FormModal
        ...
        title={viewTask?.type?.toUpperCase() || "TASK"}
        size="lg"
      >
        {viewTask && (
          <div style={{ padding: "10px 5px", color: "#1a1a1a" }}>
            <h2
              style={{ fontSize: "22px", fontWeight: 600, margin: "0 0 6px 0" }}
            >
              {viewTask.title}
            </h2>
            <div
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      viewTask.status === "Completed" ? "#10b981" : "#3b82f6",
                  }}
                />
                {viewTask.status}
              </span>
              <span>
                · <strong>{viewTask.priority || "Normal"}</strong>
              </span>
              <span
            ...
            >
              ...
              >
                {[
                  {
                    label: "Assignee",
                    value: viewTask.assignedTo?.username || "Not assigned",
                  },
                  {
                    label: "Reporter",
                    value: getCreatorName(viewTask.createdBy),
                    sub: viewTask.createdBy?.email,
                  },
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            {form.estimatedHours && !workingHours.length && (
              ...
              >
                <AlertCircle size={13} />
                Working hours not configured. Please set them in Company
                Settings.
              </div>
            )}
          </div>
