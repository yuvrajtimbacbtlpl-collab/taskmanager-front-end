...
export default function CreateIssue() {
  ...
  const [viewMode,   setViewMode]   = useState(() => localStorage.getItem("issue_view_mode") || "table");
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      let url = isGlobal
        ? `/tasks?type=issue&page=${page}&limit=${cl}&company=${companyId}`
        : `/tasks?project=${selectedProject._id}&type=issue&page=${page}&limit=${cl}${companyId?`&company=${companyId}`:""}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const taskData = {
        ...
        project: selectedProject._id, company: companyId, type:"issue", appLink:window.location.href,
  ...
  return (
    <div className="permission-page">
      ...
      <PageHeader title="Issue Management" actions={
        <div style={{display:"flex",gap:"10px",alignItems:"center",flexWrap:"wrap"}}>
          <div style={{display:"flex",border:"1.5px solid #e2e8f0",borderRadius:"9px",overflow:"hidden",background:"#f8fafc"}}>
            <button title="Table View"
              onClick={()=>{setViewMode("table");localStorage.setItem("issue_view_mode","table");}}
            ...
            <button title="Kanban View"
              onClick={()=>{setViewMode("kanban");localStorage.setItem("issue_view_mode","kanban");}}
              style={{display:"flex",alignItems:"center",gap:"5px",padding:"7px 14px",border:"none",cursor:"pointer",
                fontSize:"12px",fontWeight:600,
          ...
          {canCreate && !isGlobal && selectedProject?._id && (
            <>
              <BulkUpload type="issue" projectId={selectedProject?._id} companyId={companyId}
                onUploadSuccess={fetchTasks} onShowToast={showToast} />
              <button className="btn-primary" onClick={()=>setShowForm(true)}>+ Create Issue</button>
            </>
          )}
        </div>
      } />
      ...
      {loading ? <TableSkeleton columns={5} rows={limit} /> : viewMode === "kanban" ? (
        ...
        <CommonTable
          ...
          actions={(row)=>(
            <>
              {canUpdate && (
                <button className="iconedit" title="Edit issue"
                  onClick={e=>{
              ...
              {canDelete && (
                <button className="icondelete" title="Delete issue"
                  onClick={e=>{e.stopPropagation();setDeleteId(row._id);setDeleteName(row.title);}}>
                  <Trash2 size={18} />
                </button>
              )}
              <button className="iconview" title="View issue"
      ...
      <FormModal open={!!viewTask} onClose={()=>setViewTask(null)}
        title={viewTask?.type?.toUpperCase()||"ISSUE"} size="lg">
        {viewTask && (
      ...
      {/* ─── CREATE / EDIT MODAL ─── */}
      <FormModal open={showForm} onClose={resetForm} title={editingId?"Edit Issue":"Create Issue"}>
        <form onSubmit={handleSubmit}>

          <div style={fld}>
            <label style={lbl}>Title <span style={{color:"#dc2626"}}>*</span></label>
            <input style={inp} value={form.title} required placeholder="Enter issue title"
          ...
          <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:"10px",
            ...
            <div style={{padding:"8px 12px",background:"#eff6ff",borderRadius:"8px",
              ...
              <span style={{color:"#3b82f6",fontWeight:400}}>
                — issue starts the moment you submit, from current time
              </span>
            </div>
          ...
          <div className="form-actions" style={{marginTop:"8px"}}>
            ...
            <button type="submit" className="btn-primary" disabled={submitting}>
              {submitting?"Processing...":editingId?"Update Issue":"Create Issue"}
            </button>
          </div>
        </form>
      </FormModal>
